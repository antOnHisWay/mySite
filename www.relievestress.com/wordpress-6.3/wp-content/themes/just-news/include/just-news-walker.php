 ...
 * Class Name: just_news_navwalker
...
class Just_News_Navwalker extends Walker_Nav_Menu {
  ...
  public function start_lvl( &$output, $depth = 0, $args = array() ) {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
      ...
      if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
        ...
        $class_names = join( ' ', apply_filters( 'just_news_nav_menu_css_class', array_filter( $classes ), $item, $args, $depth ) );
        if ( $args->has_children ) {
          if ( has_nav_menu( 'categories' ) ) {
            $class_names .= ' dropdown dropright';
          }
          else{
           $class_names .= ''; 
         }
      ...
      $item_output = $args->before;
        ...
        $output      .= apply_filters( 'just_news_walker_nav_menu_start_el', $item_output, $item, $depth, $args );
      } // End if().
    }
    /**
